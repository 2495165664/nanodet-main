...
dict_ = {0: "ebike", 1:"bike"}

def write_xml(image, boxes, image_name, anno_result_path=None):
    doc = Document()
    annotation = doc.createElement('annotation')
    doc.appendChild(annotation)

    folder = doc.createElement('folder')
    folder.appendChild(doc.createTextNode('ebike'))
    ...
    for box in boxes:
        ...
        name.appendChild(doc.createTextNode(str(dict_[box[0]])))
        ...
        xmin.appendChild(doc.createTextNode(str(coordinate[1])))
        bndbox.appendChild(xmin)
        ymin = doc.createElement('ymin')
        ymin.appendChild(doc.createTextNode(str(coordinate[2])))
        bndbox.appendChild(ymin)
        xmax = doc.createElement('xmax')
        xmax.appendChild(doc.createTextNode(str(coordinate[3])))
        bndbox.appendChild(xmax)
        ymax = doc.createElement('ymax')
        ymax.appendChild(doc.createTextNode(str(coordinate[4])))
...
def overlay_bbox_cv(img, dets, class_names, score_thresh, draw=True, crop=False, xml=False, file_name=None):
    all_box = []
    for label in dets:
    ...
    for i, box in enumerate(all_box):
        ...
        text = '{}:{:.2f}%'.format(class_names[label], score * 100)
        ...
        if draw:
            ...
            cv2.rectangle(img,
                          (x0, y0 - txt_size[1] + 10),
                          (x0 + txt_size[0] + txt_size[1], y0 + 10), color, -1)
            cv2.putText(img, text, (x0, y0 + 10),
                        font, 0.5, txt_color, thickness=1)
        if crop:
            path = "./tmp_crop_bg_mp4"
            if not os.path.exists(path):
                os.mkdir(path)
            if class_names[label] == 'ebike' and score < 0.85:
                cv2.imwrite(os.path.join(path, "e{}_{:.2f}_{}.bmp".format(file_name.split(sep=".")[0], score,i)), img[y0:y1, x0:x1])
            elif class_names[label] == 'bike':
                cv2.imwrite(os.path.join(path, "b{}_{:.2f}_{}.bmp".format(file_name.split(sep=".")[0], score,i)), img[y0:y1, x0:x1])
    if xml and len(all_box) > 0:
        # path = "./auto_xml"
        # if not os.path.exists(path):
        #         os.mkdir(path)
        write_xml(img, all_box, file_name)
