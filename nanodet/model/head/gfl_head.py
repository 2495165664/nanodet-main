...
class GFLHead(nn.Module):
    ...
    def post_processN(self, preds, metas):
        cls_scores, bbox_preds = preds

        result_list = self.get_bboxesN(cls_scores, bbox_preds, metas)
        tmp = 0
        ...
        for meta in metas:
            ...
            for result, img_width, img_height, img_id, warp_matrix in zip(result_list[tmp], img_widths, img_heights, img_ids, warp_matrixes):
                det_result = {}
                det_bboxes, det_labels = result_list[tmp]
                det_bboxes = det_bboxes.cpu().numpy()
                det_bboxes[:, :4] = warp_boxes(
                    det_bboxes[:, :4], np.linalg.inv(warp_matrix), img_width, img_height
                )
    ...
    def get_bboxesN(self, cls_scores, bbox_preds, img_metas, rescale=False):
        assert len(cls_scores) == len(bbox_preds)
        num_levels = len(cls_scores)
        device = cls_scores[0].device
        ...
        result_list = []
        for img_id in range(cls_scores[0].shape[0]):
            input_height, input_width = img_metas[img_id]["img"].shape[2:]
            input_shape = [input_height, input_width]
            cls_score_list = [cls_scores[i][img_id].detach() for i in range(num_levels)]
            bbox_pred_list = [bbox_preds[i][img_id].detach() for i in range(num_levels)]
            scale_factor = 1
